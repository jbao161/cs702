...
// see if that cluster spans
    ...
    static int check_span2(int[] clusters, int[] reduced_map, int[] counts) {
        // find the largest populated cluster
        int max = 0;
        int max_cat = 0;
        int i, j, k;
...
//        System.out.println(prob + " " + avg_pterm);
    ...
    public static void main(String[] args) {
        ...
        //print_grid(clusters, x_size, y_size, p);
        int[] counts = count(clusters, x_size * y_size, cats_needed + 1);
        int span_group = check_span2(clusters, reduced_map, counts);
        randomwalk(clusters, x, y, span_group);
    }

    static void randomwalk(int[] clusters, int x_size, int y_size, int span_group) {
        //pick a random spot on the span_group
        int k = rand.nextInt();
    ...
    static void ex2() {
        ...
        for (int i = 0; i < 100; i++) {
            ...
            int[] counts = count(clusters, x_size * y_size, cats_needed + 1);
            int span_group = check_span2(clusters, reduced_map, counts);
            System.out.println(span_group + " span group");
            int size;
            for (size = 1; size < x_size; size++) {
                int count = box(size, clusters, span_group);
                counter[size] = count;
    ...
    static int box(int size, int[] grid, int span_group) {
        int rows = (int) (x_size / size + 0.5);
        int cols = (int) (y_size / size + 0.5);
        int count = 0;
        //System.out.println(rows + " " + cols + " row col");
            ...
            for (int j = 0; j < cols; j++) {
                ...
                for (int k1 = 0; k1 < size; k1++) {
                    for (int k2 = 0; k2 < size; k2++) {
                        int location = i * (y_size) * size;
                        location += j * (size);
                        location += y_size * k1;
                        location += k2;
                        if (grid[location] == span_group) {
                            count++;
                            break loop;
                        }
                    }
